...
class FourierTransformation:
    ...
    def abstract_frequency(self, data_table, cols, window_size, sampling_rate):
        ...
        # Passes over the dataset (we cannot compute it when we do not have enough history) and compute the values
        for i in range(window_size, len(data_table.index)):
            for col in cols:
                ...
                # Looks at the real part in this implementation
                for j in range(0, len(freqs)):
                    data_table.loc[
                        i, col + "_freq_" + str(freqs[j]) + "_Hz_ws_" + str(window_size)
                    ] = real_ampl[j]
                # Selects the dominant frequency, considering the positive frequencies for now
